...
<body onload="BodyLoaded();">
    <table class="wm_information wm_loading_information" style="right: auto; width: auto; top: 0px; left: 604px;" id="info_cont">
        <tr>
            <td>
...
<script type="text/javascript">
    ...
    //Used to post News 
    var idsToSend="";
    function PostNews(idToSend) {
        //idsToSend = "";
        //$($("div.wm_inbox_lines")[1]).find('input:checkbox').each(function() {
        //    if ($(this).is(':checked')) {
        //        var idEnrypted = $(this).parent().parent().attr('id');

	           
        //        var startPos = idEnrypted.indexOf('%') + 1;
        //        var endPos = idEnrypted.indexOf('#',startPos);
        //        var idToPush = idEnrypted.substring(startPos, endPos);

        //        idsToSend = idsToSend == "" ? idToPush : idsToSend + "," + idToPush;
        //       // alert(idsToSend);
        //    }
        ...
        if (idToSend == "") {
            alert("Please select email to post as news!!!");
        } else {
            window.open('http://newsindia.tv/Admin/EmailNewsActionCenter?ids='+idToSend, '_blank');
    ...
    $(document).ready(function() {
      //  $($("span.wm_toolbar_content")[2]).html("<span class='wm_toolbar_item'><span style='background-position: -480px -0px' class='wm_toolbar_icon' onclick='PostNews()' title='Post as News'>&nbsp;</span><span>Post News</span></span>" + $($("span.wm_toolbar_content")[2]).html());
    });
</script>
<% } %>
<!--<%=_WmVersion%>-->
