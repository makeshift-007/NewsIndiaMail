...
//Used to display the post button in the news Template
//
//
function InitiatePostButton(mailId) {
    //if wm_message_left !=null
    //$("span.wm_toolbar_content")[3]
    //<span class="wm_toolbar_item"><span style="background-position: -200px -0px" class="wm_toolbar_icon" title="Forward">&nbsp;</span><span class="">Forward</span></span>
    debugger;
    if ($("#postButton").length == 0) {
        var newData = $($("span.wm_toolbar_content")[3]).html() +
            "<span class='wm_toolbar_item' id='postButton' onclick='PostNews(" + mailId + ")'><span style='background-position: -200px -0px' class='wm_toolbar_icon' title='Forward'>&nbsp;</span><span class=''>Post As News</span></span>";

        $($("span.wm_toolbar_content")[3]).html(newData);
    } else {
        $("#postButton").attr('onclick', "PostNews(" + mailId + ")");
    }
}
...
CWebMail.prototype = {
    _requestMessageListNextPage: function () {
        ...
        if (screen) {
            ...
            if (lastPage >= (histObj.Page + 1)) {
                GetHandler(TYPE_MESSAGE_LIST, {
                    IdAcct: WebMail._idAcct, Page: (histObj.Page + 1), SortField: histObj.SortField,
                    SortOrder: histObj.SortOrder, FolderId: histObj.FolderId,
                    FolderFullName: histObj.FolderFullName, LookFor: histObj.LookForStr,
                    SearchFields: histObj.SearchMode
                }, [], xml, true);
            }
            else {
    ...
    _requestMessageReplyPart: function (msg, background) {
        ...
        GetHandler(TYPE_MESSAGE, {
            Id: msg.Id, Charset: msg.Charset, Uid: msg.Uid,
            FolderId: msg.FolderId, FolderFullName: msg.FolderFullName
        }, parts, xml, background);
    },
